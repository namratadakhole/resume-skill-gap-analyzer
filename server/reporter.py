...
def generate_pdf_report(analysis: Dict[str, Any], resume_name: str, job_title: str) -> bytes:
    ...
    """
    pdf = ResumeReportPDF(resume_name, job_title)
    pdf.add_page()
    ...
    pdf.multi_cell(135, 4.5, analysis['summary'], border=0)
    ...
    # Matched Skills & Missing Skills side-by-side or stacked
    pdf.set_font('Helvetica', 'B', 11)
    pdf.set_text_color(40, 167, 69) # Green
    pdf.cell(0, 6, f"Matched Skills ({len(analysis['skills']['matched'])}):", border=0)
    pdf.ln(6)
    
    pdf.set_font('Helvetica', '', 10)
    pdf.set_text_color(80, 80, 80)
    if analysis['skills']['categorized_matched']:
        for cat, skills in analysis['skills']['categorized_matched'].items():
            pdf.set_font('Helvetica', 'B', 9)
            pdf.set_text_color(60, 60, 60)
            pdf.cell(0, 5, f"{cat}:", border=0)
            pdf.ln(5)
    ...
    pdf.cell(0, 6, f"Missing Skills ({len(analysis['skills']['missing'])}):", border=0)
    pdf.ln(6)
    
    pdf.set_font('Helvetica', '', 10)
    pdf.set_text_color(80, 80, 80)
    if analysis['skills']['categorized_missing']:
        for cat, skills in analysis['skills']['categorized_missing'].items():
            pdf.set_font('Helvetica', 'B', 9)
            pdf.set_text_color(60, 60, 60)
            pdf.cell(0, 5, f"{cat}:", border=0)
            pdf.ln(5)
    ...
    for i, rec in enumerate(analysis['recommendations'], 1):
        ...
        pdf.cell(100, 5, f"{rec['skill']} ({rec['type']})", border=0)
        pdf.ln(5.5)
        ...
        pdf.set_font('Helvetica', '', 9.5)
        pdf.set_text_color(80, 80, 80)
        pdf.multi_cell(170, 4.5, rec['action'], border=0)
        pdf.ln(4)
    ...
    for check in analysis['formatting_checks']:
        name = check['name']
        status = check['status']
